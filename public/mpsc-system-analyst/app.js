...
/* Questions MPSC has printed more than once in Technical Paper I.
   Read from data/repeats.js (generated by tools/system-analyst-build/
   find_repeats.py) rather than recomputed here, so the matching rules and
   their four false-positive guards live in one reviewable place.

   What the numbers do and do not mean: `n` is the number of times a question
   appears in a REAL past paper in this bank — authored questions are excluded
   from the pool and every group spans at least two distinct sittings. That is
   observed reuse, not a forecast, and the view says so. The 17 sittings here
   are different posts (Informatics Officer, ILM, MES, Programmer, JE), so a
   count of 2 means "MPSC reused this across two of its technical papers", not
   "a 2-in-17 chance of appearing next time". Dressing that up as a percentage
   would be the same inflation provLine() exists to prevent. */
...
VIEWS.repeats = (el) => {
  ...
  const most = groups.length ? groups[0].n : 0;
  ...
  if (!groups.length) {
    el.innerHTML = `<h1>Repeats</h1><p class="empty">No repeat data. Run
      <code>python3 tools/system-analyst-build/find_repeats.py</code>.</p>`;
    return;
  }
  ...
  el.innerHTML = `
    ...
    rather than copy-pastes — it adds a blank, or writes "Which DOS command" where the older
    paper wrote "Which command". The pool is the ${pool.questions} past-paper TECH1 questions
    across ${pool.sittings} sittings; authored questions are excluded, so <strong>n is always
    a count of real printings</strong>. Treat it as observed reuse, not a prediction — those
    sittings are different posts, so "asked twice" means MPSC recycled the question between
    two of its technical papers.</p>
    <div class="grid g4 mt mb">
      ...
      ${stat('Most reprints', most + '×', 'highest count in the pool')}
    ...
    ${suspect.length ? `<div class="card mb">
      ...
      Check these against the source PDF in <code>~/Downloads/mpsc_pdfs_examination/</code> rather
      than picking whichever copy looks right.</p>
    </div>` : ''}
    ...
    <div class="row mb" style="flex-wrap:wrap">
      <label class="fld"><span>Show</span>
        <select id="rFilter">
          ...
          <option value="3">Reprinted 3+ times</option>
  ...
  const copyCard = (q, i) => q ? `
    <div class="card mb rep-copy">
      ...
      <div class="qtext">${esc(q.q)}</div>
      <div class="opts shown">${['A', 'B', 'C', 'D', 'E'].filter(k => q.opts && q.opts[k] != null).map(k => `
        <div class="opt ${q.ans === k ? 'right' : ''}" style="cursor:default">
          <span class="lab">${k}</span><span>${esc(q.opts[k])}</span></div>`).join('')}</div>
      ${q.exp || q.prov ? `<div class="expl">${q.exp ? esc(q.exp) : ''}${disputeBlock(q)}${provLine(q)}</div>` : ''}
    </div>` : '';
        ...
        <div class="spread rep-head" data-toggle="${g.gid}">
          <div style="min-width:0">
            <div class="qmeta mb">
              ...
              ${first && first.unit ? `<span class="pill">${unitLabel(first)}</span>` : ''}
              ${g.sub ? `<span class="pill">${esc(g.sub)}</span>` : ''}
            </div>
            <div class="qtext">${esc(first ? first.q : g.ids.join(' / '))}</div>
