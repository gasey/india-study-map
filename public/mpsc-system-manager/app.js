...
/* --------------------------------------------------------------- repeats */
/* Questions MPSC has printed more than once in Technical Paper I.
   Read from data/repeats.js (generated by tools/system-manager-build/
   find_repeats.py) rather than recomputed here, so the matching rules and
   their false-positive guards live in one reviewable place.

   Unlike the System Analyst app's equivalent, a rate is defensible here: all
   four sittings in the pool examine the SAME post against the SAME syllabus,
   so "printed in 3 of 4 papers" is a real base rate rather than a curiosity.
   The pool is src 'past' only — the 349 TECH1P authored practice questions and
   the 182 generated ones are excluded, because a repeat count is only evidence
   of MPSC's behaviour if every copy came from a paper MPSC actually set. */
const REPEATS = window.QUESTION_REPEATS || { groups: [], byId: {}, pool: {} };
const qsById = QS.reduce((a, q) => { a[q.id] = q; return a; }, {});

/* The three ways a group's copies can fail to line up. Kept distinct because
   only `answer` means the bank is wrong somewhere: MPSC reorders options
   between printings, so two copies can carry different answer LETTERS and
   still teach the same thing, and a stem reprinted with different distractors
   makes each copy right for its own list. Badging either of those as a
   contradiction would be exactly the overclaim provLine() guards against. */
const MISMATCH_META = {
  answer:  { pill: 'no', short: 'copies contradict',
             hint: 'The copies pick genuinely different answers from comparable options — at least one is wrong.' },
  missing: { pill: 'no', short: 'no answer recorded',
             hint: 'No copy of this question carries an answer at all.' },
  options: { pill: 'wn', short: 'different options each printing',
             hint: 'MPSC reprinted the stem with a different set of choices, so each copy\'s answer is right for its own list.' },
};
const mismatchBadge = kind => {
  const m = MISMATCH_META[kind];
  return m ? `<span class="pill ${m.pill}" title="${esc(m.hint)}">${esc(m.short)}</span>` : '';
};

VIEWS.repeats = (el) => {
  const groups = REPEATS.groups || [];
  const pool = REPEATS.pool || {};
  const total = pool.sittings || 0;
  const suspect = groups.filter(g => g.mismatch === 'answer' || g.mismatch === 'missing');
  const reprinted = groups.filter(g => g.mismatch === 'options');

  // Sub-topics ranked by how many repeated questions they account for — the
  // actual "what should I revise first" answer this view exists to give.
  const hot = {};
  groups.forEach(g => { const k = g.sub || '(untagged)'; hot[k] = (hot[k] || 0) + g.n; });
  const hotList = Object.entries(hot).sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]));

  if (!groups.length) {
    el.innerHTML = `<h1>Repeats</h1><p class="empty">No repeat data. Run
      <code>python3 tools/system-manager-build/find_repeats.py</code>.</p>`;
    return;
  }

  el.innerHTML = `
    <h1>Repeated questions</h1>
    <p class="muted">Questions MPSC has printed <strong>more than once</strong> in a real
    Technical Paper I. Matching is near-identical rather than exact, because MPSC retypes
    rather than copy-pastes — "Which command is used to see the sub-directory structure"
    became "Which DOS command…" in 2019 and is otherwise the same question. The pool is the
    ${pool.questions} past-paper TECH1 questions from all ${total} Computer Operator sittings;
    authored and practice questions are excluded, so <strong>n is always a count of real
    printings</strong>. Because all ${total} papers examine the same post against the same
    syllabus, a question in 3 of them is about as strong a signal as this bank can give you.</p>
    <div class="grid g4 mt mb">
      ${stat('Repeat groups', String(groups.length), 'distinct questions reused')}
      ${stat('Questions involved', String(groups.reduce((a, g) => a + g.n, 0)), 'copies in the bank')}
      ${stat('In 3+ sittings', String(groups.filter(g => g.sittings.length >= 3).length), 'the highest-yield set')}
      ${stat('Needs a source check', String(suspect.length), 'answer missing or contradicted')}
    </div>
    ${suspect.length ? `<div class="card mb">
      <div class="qmeta mb"><span class="pill no">needs source check</span></div>
      <p style="margin:0">${suspect.length} repeat group${suspect.length > 1 ? 's' : ''}
      cannot be trusted as ${suspect.length > 1 ? 'they' : 'it'} stand${suspect.length > 1 ? '' : 's'}.
      No official key exists for any Computer Operator paper, so every copy here is derived — and
      two derivations of one question disagreeing means at least one is wrong:
      ${suspect.map(g => `<code>${esc(g.ids.join(' / '))}</code> (${esc(MISMATCH_META[g.mismatch].short)})`).join(', ')}.
      Check against the source PDF rather than picking whichever copy looks right.</p>
    </div>` : ''}
    ${reprinted.length ? `<div class="card mb">
      <div class="qmeta mb"><span class="pill wn">not a conflict</span></div>
      <p style="margin:0">${reprinted.length} group${reprinted.length > 1 ? 's were' : ' was'} reprinted with a
      different set of options, so the copies answer different letters while each stays correct for
      its own list: ${reprinted.map(g => `<code>${esc(g.ids.join(' / '))}</code>`).join(', ')}.
      Listed so the badge is not mistaken for an error.</p>
    </div>` : ''}
    <div class="card mb">
      <div class="qmeta mb"><strong style="color:var(--ink)">Sub-topics by repeated questions</strong></div>
      <div class="row" style="flex-wrap:wrap;gap:.4rem">
        ${hotList.map(([k, v]) => `<span class="pill">${esc(k)} · ${v}</span>`).join('')}
      </div>
    </div>
    <div class="row mb" style="flex-wrap:wrap">
      <label class="fld"><span>Show</span>
        <select id="rFilter">
          ...
          <option value="3">Printed 3+ times</option>
          <option value="suspect">Needs a source check</option>
        </select></label>
      <label class="fld"><span>Sub-topic</span>
        <select id="rSub"><option value="">All sub-topics</option>
          ${hotList.map(([k]) => `<option value="${esc(k)}">${esc(k)}</option>`).join('')}
        </select></label>
      <button class="btn sm" id="rExpand">Expand all</button>
    </div>
    <div id="rList"></div>`;

  const listEl = $('#rList');

  const copyCard = (q, i) => q ? `
    <div class="card mb rep-copy">
      <div class="qmeta mb">
        <strong style="color:var(--ink)">Copy ${i + 1} · Q${q.no}</strong>
        <span class="pill">${esc(q.sitting || '')}</span>
        ${q.ans === 'COMPENSATED' ? `<span class="pill wn">voided by MPSC</span>` : ''}
      </div>
      ${passageBlock(q)}<div class="qtext">${qText(q.q)}</div>
      <div class="opts shown">${['A', 'B', 'C', 'D'].filter(k => q.opts && q.opts[k] != null).map(k => `
        <div class="opt ${q.ans === k ? 'right' : ''}" style="cursor:default">
          <span class="lab">${k}</span><span>${esc(q.opts[k])}</span></div>`).join('')}</div>
      ${q.exp || q.prov ? `<div class="expl">${q.exp ? esc(q.exp) : ''}${disputeBlock(q)}${provLine(q)}</div>` : ''}
    </div>` : '';
  ...
  const draw = () => {
    ...
    const sub = $('#rSub').value;
    const shown = groups.filter(g =>
      (f === '' || (f === 'suspect'
        ? (g.mismatch === 'answer' || g.mismatch === 'missing')
        : g.n >= +f)) &&
      (sub === '' || (g.sub || '(untagged)') === sub));

    if (!shown.length) {
      listEl.innerHTML = `<p class="empty">No repeat groups match that filter.</p>`;
      return;
    }
    listEl.innerHTML = shown.map(g => {
      const first = qsById[g.ids[0]];
      return `
      <div class="card mb rep-group" data-gid="${g.gid}">
        <div class="spread rep-head" data-toggle="${g.gid}">
          <div style="min-width:0">
            <div class="qmeta mb">
              <span class="pill acc">asked ${g.n}×</span>
              ${total ? `<span class="pill ${g.sittings.length >= 3 ? 'ok' : ''}">${g.sittings.length} of ${total} sittings</span>` : ''}
              ${mismatchBadge(g.mismatch)}
              ${g.unit ? `<span class="pill">Unit ${esc(g.unit)}</span>` : ''}
              ${g.sub ? `<span class="pill">${esc(g.sub)}</span>` : ''}
            </div>
            <div class="qtext">${first ? qText(first.q) : esc(g.ids.join(' / '))}</div>
            <p class="dim" style="margin:.35rem 0 0">Printed in: ${esc(g.sittings.join(' · '))}</p>
          </div>
          <button class="btn sm rep-btn">Show ${g.n} copies</button>
        </div>
        <div class="rep-copies" hidden>
          ${g.ids.map((id, i) => copyCard(qsById[id], i)).join('')}
        </div>
      </div>`;
    }).join('');
  };

  // One delegated handler on the container, so it survives every redraw.
  listEl.addEventListener('click', e => {
    const h = e.target.closest('[data-toggle]');
    if (!h) return;
    const box = h.parentElement.querySelector('.rep-copies');
    const open = !box.hidden;
    box.hidden = open;
    const btn = h.querySelector('.rep-btn');
    const n = box.querySelectorAll('.rep-copy').length;
    if (btn) btn.textContent = open ? `Show ${n} copies` : 'Hide copies';
  });

  $('#rExpand').onclick = () => {
    const boxes = $$('.rep-copies', listEl);
    const anyClosed = boxes.some(b => b.hidden);
    boxes.forEach(b => { b.hidden = !anyClosed; });
    $$('.rep-btn', listEl).forEach(b => {
      const n = b.closest('.rep-group').querySelectorAll('.rep-copy').length;
      b.textContent = anyClosed ? 'Hide copies' : `Show ${n} copies`;
    });
    $('#rExpand').textContent = anyClosed ? 'Collapse all' : 'Expand all';
  };
  $('#rFilter').onchange = $('#rSub').onchange = () => { draw(); $('#rExpand').textContent = 'Expand all'; };
  draw();
};
